...
/**
 * @brief 读取Excel文件的原始数据（表头 + 数据行）
 * 
 * 第一行作为表头，后续行作为数据。
 * 
 * @param filePath Excel文件路径
 * @param headers 输出参数，存储表头
 * @param data 输出参数，存储数据行
 * @return true-读取成功，false-读取失败
 */
bool ExcelReader::ReadRawData(const std::wstring& filePath,
    ...
    SAFEARRAY* pSafeArray = session.GetSafeArray();
    if (pSafeArray == NULL) {
        return false;
    }
    ...
    long colLBound = 0;
    long colUBound = 0;
    SafeArrayGetLBound(pSafeArray, 2, &colLBound);
    SafeArrayGetUBound(pSafeArray, 2, &colUBound);

    long colCount = colUBound - colLBound + 1;
    ...
    for (long col = colLBound; col <= colUBound; col++) {
        std::wstring header = session.GetCellString(lBound, col);
    ...
    for (long row = lBound + 1; row <= uBound; row++) {
        ...
        for (long col = colLBound; col <= colUBound; col++) {
            std::wstring cellValue = session.GetCellString(row, col);
